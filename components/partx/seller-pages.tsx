...
export function SellerOrderDetailPage({ id }: { id: string }) {
  ...
  return <><Link className="sx-back" href="/seller/orders"><Icon name="back"/>All seller orders</Link><div className="sx-order-detail-head"><div><span>ORDER {order.id}</span><h1>{order.productName}</h1><p>Tracking ID {order.trackingId} · Assigned to {order.storeName ?? "AutoHub Mumbai"}</p></div><Status status={order.status}/></div><div className="sx-detail-grid"><section className="sx-panel"><PanelHead title="Packing information"/><div className="sx-product-focus"><Image src={sellerOrderImage(order, sellerProducts)} alt={order.productName} width={260} height={200}/><div><span>PART TO PACK</span><h2>{order.productName}</h2><p>Part number <b>{order.partNumber}</b></p><strong>Quantity {order.quantity}</strong></div></div><dl className="sx-detail-list"><div><dt>Pack deadline</dt><dd>{order.deadline}</dd></div><div><dt>Fulfilment</dt><dd>{labelFulfilment(order.fulfilment)}</dd></div><div><dt>Payment</dt><dd className="sx-payment-detail">{paymentLabel(order)}{order.paymentReference ? <small>Ref: {order.paymentReference}</small> : null}</dd></div><div><dt>Order total</dt><dd>₹{order.total.toLocaleString("en-IN")}</dd></div></dl>{nextStatus[order.status] ? <button className="sx-primary sx-wide" onClick={() => void updateOrderStatus(order.id,nextStatus[order.status]!)}>{actionFor(order.status)}</button> : null}</section><aside className="sx-panel"><PanelHead title="Customer & delivery"/><div className="sx-customer-card"><div className="sx-avatar">{initials(order.customer.name)}</div><div><b>{order.customer.name}</b><span>{order.customer.phone}</span><small>{maskEmail(order.customer.email)}</small></div></div><div className="sx-contact-actions"><a href={`tel:${order.customer.phone}`}><Icon name="phone"/>Call customer</a><button onClick={() => navigator.clipboard?.writeText(order.customer.phone)}><Icon name="copy"/>Copy number</button></div><p className="sx-privacy-note">Customer details are shown only for fulfilment and order support.</p><Link className="sx-secondary sx-wide" href={`/seller/tickets?order=${order.id}`}><Icon name="ticket"/>View related tickets</Link></aside></div></>;
}
...
function sellerOrderImage(order: SellerOrder, sellerProducts: SellerProduct[]) {
  if (order.imageUrl) return order.imageUrl;
  const inventoryProduct = sellerProducts.find((product) => product.id === order.productId || product.partNumber === order.partNumber);
  if (inventoryProduct?.imageUrl) return inventoryProduct.imageUrl;
  const demoProduct = getDemoCatalog().find((product) => product.id === order.productId || product.partNumber === order.partNumber);
  if (demoProduct?.imageUrl) return demoProduct.imageUrl;
  const imageIndex = order.imageIndex ?? demoProduct?.imageIndex ?? 0;
