...
export function SellerOrderDetailPage({ id }: { id: string }) {
  ...
  return <><Link className="sx-back" href="/seller/orders"><Icon name="back"/>All seller orders</Link><div className="sx-order-detail-head"><div><span>ORDER {order.id}</span><h1>{order.productName}</h1><p>Tracking ID {order.trackingId} · Assigned to {order.storeName ?? "AutoHub Mumbai"}</p></div><Status status={order.status}/></div><div className="sx-detail-grid"><section className="sx-panel"><PanelHead title="Packing information"/><div className="sx-product-focus"><Image src="/parts/0-v2.png" alt={order.productName} width={260} height={200}/><div><span>PART TO PACK</span><h2>{order.productName}</h2><p>Part number <b>{order.partNumber}</b></p><strong>Quantity {order.quantity}</strong></div></div><dl className="sx-detail-list"><div><dt>Pack deadline</dt><dd>{order.deadline}</dd></div><div><dt>Fulfilment</dt><dd>{labelFulfilment(order.fulfilment)}</dd></div><div><dt>Payment</dt><dd>{order.paymentStatus}</dd></div><div><dt>Order total</dt><dd>₹{order.total.toLocaleString("en-IN")}</dd></div></dl>{nextStatus[order.status] ? <button className="sx-primary sx-wide" onClick={() => void updateOrderStatus(order.id,nextStatus[order.status]!)}>{actionFor(order.status)}</button> : null}</section><aside className="sx-panel"><PanelHead title="Customer & delivery"/><div className="sx-customer-card"><div className="sx-avatar">{initials(order.customer.name)}</div><div><b>{order.customer.name}</b><span>{order.customer.phone}</span><small>{maskEmail(order.customer.email)}</small></div></div><div className="sx-contact-actions"><a href={`tel:${order.customer.phone}`}><Icon name="phone"/>Call customer</a><button onClick={() => navigator.clipboard?.writeText(order.customer.phone)}><Icon name="copy"/>Copy number</button></div><p className="sx-privacy-note">Customer details are shown only for fulfilment and order support.</p><Link className="sx-secondary sx-wide" href={`/seller/tickets?order=${order.id}`}><Icon name="ticket"/>View related tickets</Link></aside></div></>;
}
...
function SellerOrderTable({ orders, all=false }: { orders: SellerOrder[]; all?: boolean }) {
  return <div className="sx-table-wrap"><div className="sx-order-table sx-order-head"><span>Order ID</span><span>Placed at</span><span>Customer</span><span>Product</span><span>Qty</span><span>Fulfilment</span><span>Payment</span><span>{all?"Status":"Deadline"}</span><span>Action</span></div>{orders.map((order) => <div className="sx-order-table" key={order.id}><b>{order.id}</b><span>{order.placedAt}</span><span><b>{order.customer.name}</b><small>{order.customer.phone}</small></span><span><b>{order.productName}</b><small>{order.partNumber} · {order.storeName ?? "AutoHub Mumbai"}</small></span><span>{order.quantity}</span><span>{labelFulfilment(order.fulfilment)}</span><span className={`payment-${order.paymentStatus.toLowerCase()}`}>{order.paymentStatus}</span><span>{all?<Status status={order.status}/>:order.deadline}</span><Link className="sx-primary" href={`/seller/orders/${order.id}`}>Open order</Link></div>)}</div>;
}
