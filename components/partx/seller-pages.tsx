...
import type { NewSellerProduct, SellerOrder, SellerOrderItem, SellerOrderStatus, SellerProduct, StorePaymentSettings } from "@/lib/types";
import { getDemoCatalog } from "@/lib/demo-data";
import { usePartX } from "./app-provider";
import { Icon } from "./icons";
import { sellerOrderHref } from "@/lib/navigation";
...
export function SellerDashboardPage() {
  ...
  return <>
    ...
    <div className="sx-lower-grid">
      ...
      <section className="sx-panel sx-rating-summary"><PanelHead title="Verified ratings" href="/seller/reviews" link="View all reviews"/><div><strong>{rating.count ? rating.average.toFixed(1) : "New"}{rating.count ? <small>/ 5</small> : null}</strong><div className="sx-stars">{rating.stars}</div><span>{rating.count ? `Based on ${rating.count} verified ${rating.count === 1 ? "review" : "reviews"}` : "No verified ratings yet"}</span></div><div className="sx-rating-bars">{rating.distribution.map(({ star, percentage }) => <div key={star}><b>{star} ★</b><i><span style={{width:`${percentage}%`}}/></i><em>{percentage}%</em></div>)}</div><small>Across all stores on this seller account</small></section>
    </div>
  </>;
}
...
export function SellerPaymentsPage() {
  ...
  return <><SellerTitle title="Payments awaiting verification" subtitle="Confirm direct UPI receipts before orders enter the packing queue"/>{message ? <div className="sx-product-success" role="status"><Icon name="check"/>{message}</div> : null}{error ? <p className="sx-form-error" role="alert">{error}</p> : null}<section className="sx-payment-verification-list">{paymentVerifications.map((order) => <article className="sx-panel" key={order.id}><header><div><span>PARTX ORDER</span><h2>{order.id}</h2><p>{order.paymentSubmittedAt ?? order.placedAt}</p></div><Status status={order.paymentStatus === "VERIFICATION_FAILED" ? "Payment not found" : "Awaiting verification"}/></header><div className="sx-payment-amount"><span>Amount customer says they paid</span><strong>₹{order.total.toLocaleString("en-IN")}</strong><small>Direct to {order.sellerUpiNameSnapshot ?? order.storeName}</small></div><dl><div><dt>UPI reference / UTR</dt><dd>{order.paymentReference ?? "Not submitted"}</dd></div><div><dt>Customer</dt><dd>{order.customer.name}<small>{order.customer.phone}</small></dd></div><div><dt>Order items</dt><dd className="sx-payment-order-items">{sellerOrderItems(order).map((item) => <span key={`${item.productId ?? item.partNumber}-${item.productName}`}>{item.productName}<small>{item.partNumber} · Qty {item.quantity}</small></span>)}<small>{labelFulfilment(order.fulfilment)}</small></dd></div></dl>{order.paymentStatus === "PAYMENT_SUBMITTED" ? <footer><button className="sx-secondary" disabled={workingId === order.id} onClick={() => void act(order.id,"missing")}>Payment not found</button><button className="sx-primary" disabled={workingId === order.id} onClick={() => void act(order.id,"confirm")}>{workingId === order.id ? "Updating…" : "Confirm payment received"}</button></footer> : <footer><span>Waiting for the customer to submit a corrected reference.</span></footer>}</article>)}{!paymentVerifications.length ? <div className="sx-panel sx-payment-empty"><Icon name="check"/><h2>No payments waiting</h2><p>Submitted UPI references for this store will appear here in real time.</p></div> : null}</section></>;
}

export function SellerPackingPage() {
  const { sellerOrders, sellerProducts, updateOrderStatus } = useSeller();
  const queue = sellerOrders.filter((order) => ["New","Accepted","Packing","Packed"].includes(order.status));
  return <><SellerTitle title="Packing queue" subtitle="Pack every listed item before each fulfilment deadline"/><div className="sx-packing-list">{queue.map((order, index) => <article key={order.id}><div className="sx-pack-number">{String(index + 1).padStart(2,"0")}</div><div className="sx-pack-items"><span className="sx-pack-order-id">{order.id} · {sellerOrderItems(order).length} product{sellerOrderItems(order).length === 1 ? "" : "s"}</span>{sellerOrderItems(order).map((item) => <div className="sx-pack-item" key={`${item.productId ?? item.partNumber}-${item.productName}`}><Image src={sellerOrderItemImage(item, sellerProducts)} alt={item.productName} width={90} height={70}/><div><h2>{item.productName}</h2><p>{item.partNumber || "No part number"} · Quantity <b>{item.quantity}</b></p></div></div>)}<small>Tracking {order.trackingId}</small></div><div className="sx-pack-meta"><span>Fulfilment<b>{labelFulfilment(order.fulfilment)}</b></span><span>Pack by<b className="urgent">{order.deadline}</b></span><span>Customer<b>{order.customer.name}</b></span></div><div className="sx-pack-actions"><Status status={order.status}/><Link href={sellerOrderHref(order.id)}>Open order</Link>{nextStatus[order.status] ? <button className="sx-primary" onClick={() => void updateOrderStatus(order.id,nextStatus[order.status]!)}>{actionFor(order.status)}</button> : null}</div></article>)}{!queue.length ? <div className="sx-queue-empty"><Icon name="box"/><h2>No orders waiting to be packed</h2><p>New paid or cash-on-delivery orders assigned to this store will appear here in real time.</p></div> : null}</div></>;
}

export function SellerOrderDetailPage({ id }: { id: string }) {
  const { sellerOrders, sellerProducts, updateOrderStatus } = useSeller(); const order = sellerOrders.find((item) => item.id === id);
  if (!order) return <div className="sx-empty"><h1>Order not found</h1><Link href="/seller/orders">Return to orders</Link></div>;
  const items = sellerOrderItems(order);
  return <><Link className="sx-back" href="/seller/orders"><Icon name="back"/>All seller orders</Link><div className="sx-order-detail-head"><div><span>ORDER {order.id}</span><h1>{items.length} product{items.length === 1 ? "" : "s"} to pack</h1><p>Tracking ID {order.trackingId} · Assigned to {order.storeName ?? "AutoHub Mumbai"}</p></div><Status status={order.status}/></div><div className="sx-detail-grid"><section className="sx-panel"><PanelHead title="Packing information"/><div className="sx-product-focus-list">{items.map((item, index) => <div className="sx-product-focus" key={`${item.productId ?? item.partNumber}-${item.productName}`}><Image src={sellerOrderItemImage(item, sellerProducts)} alt={item.productName} width={260} height={200}/><div><span>ITEM {index + 1} OF {items.length}</span><h2>{item.productName}</h2><p>Part number <b>{item.partNumber || "Not provided"}</b></p><strong>Quantity {item.quantity}</strong></div></div>)}</div><dl className="sx-detail-list"><div><dt>Pack deadline</dt><dd>{order.deadline}</dd></div><div><dt>Fulfilment</dt><dd>{labelFulfilment(order.fulfilment)}</dd></div><div><dt>Payment</dt><dd className="sx-payment-detail">{paymentLabel(order)}{order.paymentReference ? <small>Ref: {order.paymentReference}</small> : null}</dd></div><div><dt>Order total</dt><dd>₹{order.total.toLocaleString("en-IN")}</dd></div></dl>{nextStatus[order.status] ? <button className="sx-primary sx-wide" onClick={() => void updateOrderStatus(order.id,nextStatus[order.status]!)}>{actionFor(order.status)}</button> : null}</section><aside className="sx-panel"><PanelHead title="Customer & delivery"/><div className="sx-customer-card"><div className="sx-avatar">{initials(order.customer.name)}</div><div><b>{order.customer.name}</b><span>{order.customer.phone}</span><small>{maskEmail(order.customer.email)}</small></div></div><div className="sx-contact-actions"><a href={`tel:${order.customer.phone}`}><Icon name="phone"/>Call customer</a><button onClick={() => navigator.clipboard?.writeText(order.customer.phone)}><Icon name="copy"/>Copy number</button></div><p className="sx-privacy-note">Customer details are shown only for fulfilment and order support.</p><Link className="sx-secondary sx-wide" href={`/seller/tickets?order=${order.id}`}><Icon name="ticket"/>View related tickets</Link></aside></div></>;
}

export function SellerTicketsPage() {
  const { tickets, resolveTicket } = useSeller(); const [tab,setTab] = useState("Open"); const [selectedId,setSelectedId] = useState(tickets.find((ticket) => ticket.status === "Open")?.id ?? tickets[0]?.id); const [note,setNote] = useState(""); const [resolved,setResolved] = useState(false); const [closing,setClosing]=useState(false); const [error,setError]=useState("");
...
export function SellerReviewsPage() {
  ...
  return <><SellerTitle title="Verified customer ratings" subtitle={`Ratings submitted across ${user?.storeIds?.length ?? 0} seller ${user?.storeIds?.length === 1 ? "store" : "stores"}`}/><div className="sx-reviews-layout"><section className="sx-panel sx-rating-hero"><strong>{summary.count ? summary.average.toFixed(1) : "New"}{summary.count ? <small>/ 5</small> : null}</strong><div className="sx-stars">{summary.stars}</div><p>{summary.count ? `${summary.count} verified store ${summary.count === 1 ? "rating" : "ratings"}` : "No verified ratings yet"}</p><span>Displayed during customer store selection</span></section><section className="sx-review-list">{storeRatings.map((rating) => <article className="sx-panel" key={rating.id}><div><b>{rating.customerName}</b><span>{"★".repeat(rating.stars)}{"☆".repeat(5-rating.stars)}</span></div><p>{rating.comment || "No written comment."}</p><small><Icon name="check"/>Verified purchase · {rating.storeName} · {rating.orderId} · {rating.createdAt}</small></article>)}{!storeRatings.length ? <div className="sx-empty-row">No verified customer ratings for your stores yet.</div> : null}</section></div></>;
}
...
function SellerOrderTable({ orders, all=false }: { orders: SellerOrder[]; all?: boolean }) {
  return <div className="sx-table-wrap"><div className="sx-order-table sx-order-head"><span>Order ID</span><span>Placed at</span><span>Customer</span><span>Products to pack</span><span>Total qty</span><span>Fulfilment</span><span>Payment</span><span>{all?"Status":"Deadline"}</span><span>Action</span></div>{orders.map((order) => <div className="sx-order-table" key={order.id}><b>{order.id}</b><span>{order.placedAt}</span><span><b>{order.customer.name}</b><small>{order.customer.phone}</small></span><span className="sx-order-items">{sellerOrderItems(order).map((item) => <span key={`${item.productId ?? item.partNumber}-${item.productName}`}><b>{item.productName}</b><small>{item.partNumber || "No part number"} · Qty {item.quantity}</small></span>)}<small>{order.storeName ?? "PartX seller"}</small></span><span>{sellerOrderItems(order).reduce((total, item) => total + item.quantity, 0)}</span><span>{labelFulfilment(order.fulfilment)}</span><span className={`payment-${order.paymentStatus.toLowerCase()}`}>{paymentLabel(order)}{order.paymentReference ? <small>{order.paymentReference}</small> : null}</span><span>{all?<Status status={order.status}/>:order.deadline}</span><Link className="sx-primary" href={sellerOrderHref(order.id)}>Open order</Link></div>)}{!orders.length ? <div className="sx-empty-row">No real orders have been assigned to this store yet.</div> : null}</div>;
}
...
function sellerOrderItems(order: SellerOrder): SellerOrderItem[] {
...
function sellerOrderItemImage(item: SellerOrderItem, sellerProducts: SellerProduct[]) {
  if (item.imageUrl) return item.imageUrl;
  const inventoryProduct = sellerProducts.find((product) => product.id === item.productId || product.partNumber === item.partNumber);
  if (inventoryProduct?.imageUrl) return inventoryProduct.imageUrl;
  const demoProduct = getDemoCatalog().find((product) => product.id === item.productId || product.partNumber === item.partNumber);
  if (demoProduct?.imageUrl) return demoProduct.imageUrl;
  const imageIndex = item.imageIndex ?? demoProduct?.imageIndex ?? 0;
  return `/parts/${Math.max(0, Math.min(imageIndex, 5))}-v2.png`;
}
