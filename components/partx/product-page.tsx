...
export function ProductPage({ product }: { product: Product }) {
  const { addToCart, activeVehicleId, vehicles, stores, catalog } = usePartX();
  const vehicle = vehicles.find((item) => item.id === activeVehicleId);
  const sellerOptions = useMemo(() => {
    const hasRealListings = stores.some((store) => store.listings.some((listing) => listing.productId === product.id || listing.partNumber === product.partNumber));
    return stores.flatMap((store, index) => {
      const listing = store.listings.find((item) => item.productId === product.id || item.partNumber === product.partNumber);
      if (!listing && hasRealListings) return [];
      const listedProduct = listing ? catalog.find((item) => item.id === listing.productId) ?? product : product;
      return [{ store, product: listedProduct, price: listing?.price ?? product.price + index * 50, stock: listing?.stock ?? product.stock }];
    }).sort((a, b) => a.price - b.price);
  }, [catalog, product, stores]);
  ...
  return <div className="px-page px-container">
    ...
    <div className="px-detail-grid">
      <div className="px-detail-image"><Image src={selectedProduct.imageUrl ?? `/parts/${selectedProduct.imageIndex}-v2.png`} alt={selectedProduct.name} width={900} height={720} priority/><span>{selectedProduct.kind}</span></div>
    ...
    <section className="px-subsection"><div className="px-section-head"><div><span>COMPARE SELLERS</span><h2>Available stores</h2></div></div>
      <div className="px-seller-list">{sellerOptions.map(({ store, price, stock }, index) => { const isSelected = store.id === selected?.store.id; return <button type="button" className={isSelected ? "active" : ""} aria-pressed={isSelected} onClick={() => setSelectedStoreId(store.id)} key={store.id}><i className="px-store-radio">{isSelected ? <Icon name="check"/> : null}</i><div><b>{store.name}{index === 0 ? <em>LOWEST PRICE</em> : null}</b><span>★ {store.rating} · {store.ratingCount ?? 0} verified ratings · {store.distanceKm} km</span></div><strong>₹{price.toLocaleString("en-IN")}</strong><span>{stock} in stock</span><small>{isSelected ? "Selected seller" : "Choose seller"}</small></button>; })}</div>
    </section>
  </div>;
}

export function CatalogProductPage({ id }: { id: string }) {
  const { catalog } = usePartX();
  const product = catalog.find((item) => item.id === id);
  if (!product) return <div className="px-page px-container px-empty"><h2>Loading product…</h2><p>If this product was removed by the seller, return to the catalog.</p><Link href="/shop" className="px-btn px-btn-dark">Back to parts</Link></div>;
