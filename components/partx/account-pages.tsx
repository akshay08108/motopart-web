...
export function OrderDetailPage({ id }: { id: string }) {
  ...
  return <div className="px-page px-container">
    ...
    <div className="px-dashboard-grid"><section className="px-panel px-panel-wide"><span>ORDER DETAILS</span><h2>{order.items?.length ?? 1} item{(order.items?.length ?? 1) > 1 ? "s" : ""} · ₹{order.total.toLocaleString("en-IN")}</h2><p>Tracking ID: {order.trackingId ?? "PX-TRK-78451236"} · Fulfilment: {order.fulfilment ?? "delivery"} · Seller: {order.storeName ?? "AutoHub Mumbai"}</p>{order.stage === "Delivered" ? <button className="px-btn px-btn-red" onClick={() => setRatingDismissed(false)}>{rated || alreadyRated ? "Rating submitted" : "Rate this store"}</button> : null}</section><section className="px-panel"><span>NEED HELP?</span><h2>We’re here for this order</h2><p>Report delivery, fitment, payment, return, or wrong-part issues.</p><Link href={`/support?order=${order.id}`} className="px-btn px-btn-outline">Contact us</Link></section></div>
...
export function AccountPage() {
  ...
  return <div className="px-page px-container"><div className="px-account-hero"><div className="px-avatar">{initials}</div><div><span>PARTX CUSTOMER</span><h1>{user?.name}</h1><p>{user?.email} · {user?.mobile}</p></div><div className="px-account-actions"><Link href="/seller" className="px-btn px-btn-dark"><Icon name="store"/>Switch to Seller</Link><button className="px-btn px-btn-outline" onClick={() => { signOut(); router.replace("/login"); }}><Icon name="logout"/>Sign out</button></div></div><div className="px-account-links">{links.map(([href, icon, title, subtitle]) => <Link href={href} key={href}><span className="px-square-icon"><Icon name={icon}/></span><div><b>{title}</b><span>{subtitle}</span></div><Icon name="chevron"/></Link>)}</div></div>;
}
...
function RatingDialog({orderId,storeId,close,submit}:{orderId:string;storeId:string;close:()=>void;submit:(stars:number,comment:string)=>void}){
  const [stars,setStars]=useState(5);const [comment,setComment]=useState("");
  return <div className="px-modal-backdrop"><div className="px-modal px-rating-dialog" role="dialog" aria-modal="true" aria-label="Rate your store"><div className="px-modal-head"><div><span>DELIVERY COMPLETE</span><h2>How was AutoHub Mumbai?</h2></div><button onClick={close} aria-label="Rate later"><Icon name="close"/></button></div><p>Your verified rating will appear when customers choose a seller.</p><div className="px-rating-stars" aria-label={`${stars} stars`}>{[1,2,3,4,5].map((value)=><button className={value<=stars?"active":""} onClick={()=>setStars(value)} key={value} aria-label={`${value} stars`}><Icon name="star"/></button>)}</div><label>Share an optional comment<textarea value={comment} onChange={(event)=>setComment(event.target.value)} placeholder="Part quality, packing and delivery experience"/></label><div className="px-rating-actions"><button className="px-btn px-btn-outline" onClick={close}>Rate later</button><button className="px-btn px-btn-red" onClick={()=>submit(stars,comment)}>Submit verified rating</button></div><small>Order {orderId} · Store {storeId}</small></div></div>;
}
