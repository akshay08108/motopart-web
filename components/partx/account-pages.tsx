...
export function AccountPage() {
  ...
  return <div className="px-page px-container"><div className="px-account-hero"><div className="px-avatar">AS</div><div><span>PARTX CUSTOMER</span><h1>Akshay Singh</h1><p>Mumbai · Demo profile</p></div><Link href="/seller" className="px-btn px-btn-dark px-seller-switch"><Icon name="logout"/>Switch to Seller</Link></div><div className="px-account-links">{links.map(([href, icon, title, subtitle]) => <Link href={href} key={href}><span className="px-square-icon"><Icon name={icon}/></span><div><b>{title}</b><span>{subtitle}</span></div><Icon name="chevron"/></Link>)}</div></div>;
}
