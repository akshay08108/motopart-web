...
export function OrderDetailPage({ id }: { id: string }) {
  const { orders } = usePartX(); const order = orders.find((item) => item.id === id);
...
export function AccountPage() {
  const { location, vehicles, orders } = usePartX();
  ...
  return <div className="px-page px-container"><div className="px-account-hero"><div className="px-avatar">AS</div><div><span>PARTX CUSTOMER</span><h1>Akshay Singh</h1><p>Mumbai · Demo profile</p></div></div><div className="px-account-links">{links.map(([href, icon, title, subtitle]) => <Link href={href} key={href}><span className="px-square-icon"><Icon name={icon}/></span><div><b>{title}</b><span>{subtitle}</span></div><Icon name="chevron"/></Link>)}</div></div>;
}

export function SupportPage() {
  const params = useSearchParams(); const { orders } = usePartX(); const [sent, setSent] = useState(false);
  return <div className="px-page px-container"><div className="px-page-title"><span>PARTX SUPPORT</span><h1>How can we help?</h1><p>Tell us what happened and tie it to an order for a faster response.</p></div><div className="px-support-layout"><form className="px-panel px-form" onSubmit={(event) => { event.preventDefault(); setSent(true); }}><label>Order<select defaultValue={params.get("order") ?? ""}><option value="">General question</option>{orders.map((order) => <option value={order.id} key={order.id}>{order.id} · {order.stage}</option>)}</select></label><label>Issue type<select><option>Delivery issue</option><option>Wrong part</option><option>Fitment issue</option><option>Payment issue</option><option>Return or refund</option><option>Other</option></select></label><label>Tell us what happened<textarea required placeholder="Include any details that will help our support team."/></label><button className="px-btn px-btn-red" type="submit">{sent ? "Ticket created · PX-SUPPORT-104" : "Submit support request"}</button></form><aside className="px-support-aside"><div><Icon name="headset"/><h2>Order-specific help</h2><p>Support requests are stored locally in this demo and can later post to your support API.</p></div><div><b>Typical response</b><strong>Under 2 hours</strong><span>Monday–Saturday, 9 AM–8 PM</span></div></aside></div></div>;
}
