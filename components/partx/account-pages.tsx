...
export function CheckoutPage() {
  ...
  const submitReference = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!attempt || !upiAppOpened || !paymentConfirmed || !isValidUtr(transactionReference)) { setCheckoutError("Open a UPI app, complete the payment, confirm success, then enter the 12–35 digit bank UTR."); return; }
    setProcessing(true); setCheckoutError("");
    try {
  ...
  const cancel = async () => {
    ...
    try {
      ...
      removeBrowserStorage("session", attemptKey);
      setAttempt(null); setShowQr(false); setTransactionReference(""); setUpiAppOpened(false); setPaymentConfirmed(false);
    } catch (reason) {
  ...
  return <div className="px-page px-container"><Link className="px-back-link" href="/cart"><Icon name="back"/>Back to cart</Link><div className="px-page-title"><span>SECURE CHECKOUT</span><h1>Choose how you receive it</h1><p>Pay the selected seller directly by UPI or choose cash on delivery.</p></div>
    <div className="px-checkout-layout"><div className="px-checkout-main"><section className="px-panel"><div className="px-step-title"><b>1</b><div><span>FULFILMENT</span><h2>Delivery, pickup or garage</h2></div></div><div className="px-choice-grid">{(["delivery", "pickup", "garage"] as FulfilmentMode[]).map((mode) => <button className={fulfilment === mode ? "active" : ""} onClick={() => setFulfilment(mode)} key={mode}><Icon name={mode === "delivery" ? "pin" : mode === "pickup" ? "orders" : "garage"}/><b>{mode === "delivery" ? "Deliver to me" : mode === "pickup" ? "Self pickup" : "Send to garage"}</b><span>{mode === "delivery" ? location.address : mode === "pickup" ? `${selectedStore.name} · ${selectedStore.distanceKm} km` : garages[0]?.name ?? "Add a garage first"}</span></button>)}</div></section>
      <section className="px-panel"><div className="px-step-title"><b>2</b><div><span>PAYMENT</span><h2>Pay the seller directly</h2></div></div>{enabledMethods.length ? <div className="px-payment-row">{enabledMethods.map((method) => <button className={selectedPayment === method ? "active" : ""} disabled={Boolean(attempt)} onClick={() => setPayment(method)} key={method}>{method === "upi" ? "UPI" : "Cash on delivery"}</button>)}</div> : <div className="px-payment-unavailable" role="alert">{selectedStore.name} has not enabled UPI or cash on delivery. Contact the seller before ordering.</div>}{selectedPayment === "upi" && !attempt ? <div className="px-upi"><b>Direct seller payment</b><span>Your payment goes to {selectedStore.name}. PartX does not collect or hold the money.</span></div> : null}{attempt && upiUri ? <div className="px-upi-payment"><div className="px-upi-heading"><span>PAYMENT ATTEMPT {attempt.id}</span><h3>Paying ₹{attempt.total.toLocaleString("en-IN")} to {attempt.sellerUpiNameSnapshot}</h3><p>UPI ID <b>{attempt.sellerUpiIdSnapshot}</b><button type="button" onClick={() => navigator.clipboard?.writeText(attempt.sellerUpiIdSnapshot ?? "")}><Icon name="copy"/>Copy</button></p></div>{showQr ? <div className="px-upi-qr"><QRCodeSVG value={upiUri} size={210} level="M" marginSize={2}/><small>Exact amount · INR · Reference {attempt.id}</small></div> : null}<div className="px-upi-apps"><b>Choose a UPI app</b><div><button type="button" onClick={() => launchUpi("com.google.android.apps.nbu.paisa.user")}>Google Pay</button><button type="button" onClick={() => launchUpi("com.phonepe.app")}>PhonePe</button><button type="button" onClick={() => launchUpi("net.one97.paytm")}>Paytm</button><button type="button" onClick={() => launchUpi("in.org.npci.upiapp")}>BHIM</button></div><small>Use a verified merchant UPI ID. A personal VPA may be flagged or declined by the selected payment app.</small></div><div className="px-upi-actions"><button type="button" className="px-btn px-btn-red" onClick={() => launchUpi()}>Open installed UPI apps <Icon name="arrow"/></button><button type="button" className="px-btn px-btn-outline" onClick={() => setShowQr((shown) => !shown)}>{showQr ? "Hide QR code" : "Show QR code"}</button></div><form className="px-utr-form" onSubmit={submitReference}><h3>Payment completed?</h3><p>Only continue after the UPI app shows success. Copy the bank UTR—not the PartX order ID—from the payment details.</p><label>Bank UTR (12–35 digits)<input value={transactionReference} onChange={(event) => setTransactionReference(event.target.value.replace(/\D/g, "").slice(0, 35))} autoComplete="off" inputMode="numeric" pattern="[0-9]{12,35}" minLength={12} maxLength={35} required placeholder="e.g. 423456789012"/></label><label className="px-payment-confirm"><input type="checkbox" checked={paymentConfirmed} onChange={(event) => setPaymentConfirmed(event.target.checked)}/>I confirm my UPI app showed this payment as successful.</label><div><button className="px-btn px-btn-red" type="submit" disabled={processing || !upiAppOpened || !paymentConfirmed || !isValidUtr(transactionReference)}>{processing ? "Submitting…" : "Send for seller verification"}</button><button className="px-btn px-btn-outline" type="button" onClick={() => launchUpi()}>Try another UPI app</button><button className="px-link-button" type="button" onClick={() => void cancel()}>Cancel</button></div><small>This is still manual verification. The seller must confirm the money in their bank or UPI statement before accepting the order.</small></form></div> : null}</section></div>
...
export function OrderDetailPage({ id }: { id: string }) {
  ...
  const { orders, liveOrderUpdate, submitUpiReference } = usePartX();
  ...
  const ratingOpen = order?.stage === "Delivered" && Boolean(order.storeId) && !alreadyRated && !ratingDismissed;
  ...
  return <div className="px-page px-container">
    ...
    {ratingOpen && order.storeId ? <RatingDialog orderId={order.id} storeId={order.storeId} storeName={order.storeName ?? "your selected store"} close={() => setRatingDismissed(true)} submit={async (stars, comment) => { await addRating({ orderId: order.id, storeId: order.storeId!, stars, comment }); setRated(true); setRatingDismissed(true); }}/> : null}
  </div>;
}
...
export function AccountPage() {
  const router = useRouter();
  const { location, vehicles, orders, user, signOut, notificationPermission, notificationError, enableNotifications } = usePartX();
  ...
  return <div className="px-page px-container"><div className="px-account-hero"><div className="px-avatar">{initials}</div><div><span>PARTX CUSTOMER</span><h1>{user?.name}</h1><p>{user?.email} · {user?.mobile}</p>{notificationError ? <small className="px-notification-error" role="alert">{notificationError}</small> : null}</div><div className="px-account-actions">{notificationPermission !== "unsupported" ? <button className={notificationPermission === "granted" ? "px-btn px-btn-dark" : "px-btn px-btn-red"} onClick={() => void enableNotifications()}><Icon name="bell"/>{notificationPermission === "granted" ? "Notifications enabled" : "Enable notifications"}</button> : null}{user?.roles.includes("seller") ? <Link href="/seller" className="px-btn px-btn-dark"><Icon name="store"/>Switch to Seller</Link> : null}<button className="px-btn px-btn-outline" onClick={() => { void signOut().then(() => router.replace("/login")); }}><Icon name="logout"/>Sign out</button></div></div><div className="px-account-links">{links.map(([href, icon, title, subtitle]) => <Link href={href} key={href}><span className="px-square-icon"><Icon name={icon}/></span><div><b>{title}</b><span>{subtitle}</span></div><Icon name="chevron"/></Link>)}</div></div>;
}

export function SupportPage() {
  const params = useSearchParams(); const { orders } = usePartX(); const { addTicket, tickets } = useSeller(); const [ticketId, setTicketId] = useState(""); const [submitting,setSubmitting]=useState(false); const [error,setError]=useState("");
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form=event.currentTarget; const data=new FormData(form); setSubmitting(true); setError(""); setTicketId(""); try { const ticket=await addTicket({orderId:String(data.get("order")),issue:String(data.get("issue")),message:String(data.get("message"))}); setTicketId(ticket.id); form.reset(); } catch (reason) { setError(reason instanceof Error ? reason.message : "The support request could not be created."); } finally { setSubmitting(false); } };
  return <div className="px-page px-container"><div className="px-page-title"><span>PARTX SUPPORT</span><h1>How can we help?</h1><p>Choose an order so your request reaches the correct store immediately.</p></div><div className="px-support-layout"><form className="px-panel px-form" onSubmit={(event) => void submit(event)}><label>Order<select name="order" defaultValue={params.get("order") ?? ""} required><option value="" disabled>Select an order</option>{orders.map((order) => <option value={order.id} key={order.id}>{order.id} · {order.storeName ?? "PartX seller"} · {order.stage}</option>)}</select></label><label>Issue type<select name="issue"><option>Delivery issue</option><option>Wrong part received</option><option>Fitment issue</option><option>Payment issue</option><option>Return or refund</option><option>Other</option></select></label><label>Tell us what happened<textarea name="message" minLength={10} maxLength={1000} required placeholder="Include any details that will help the seller."/></label><button className="px-btn px-btn-red" type="submit" disabled={submitting}>{submitting ? "Sending to seller…" : "Submit support request"}</button>{ticketId?<p className="px-ticket-success"><Icon name="check"/>Ticket {ticketId} was sent to the seller in real time.</p>:null}{error?<p className="px-checkout-error" role="alert">{error}</p>:null}</form><aside className="px-support-aside"><div><Icon name="headset"/><h2>Order-specific help</h2><p>Your request and its status stay linked to the store that fulfilled the selected order.</p></div><div><b>Typical response</b><strong>Under 2 hours</strong><span>Monday–Saturday, 9 AM–8 PM</span></div></aside></div><section className="px-support-history"><span>MY SUPPORT REQUESTS</span><h2>Ticket history</h2>{tickets.map((ticket)=><article className="px-panel" key={ticket.id}><div><b>{ticket.issue}</b><span>{ticket.status}</span></div><p>{ticket.message}</p><small>{ticket.storeName} · Order {ticket.orderId} · {ticket.createdAt}</small>{ticket.status === "Resolved" ? <em>Resolved by seller{ticket.resolvedAt ? ` · ${ticket.resolvedAt}` : ""}</em> : null}</article>)}{!tickets.length?<p className="px-panel">No support requests yet.</p>:null}</section></div>;
}
...
function RatingDialog({orderId,storeId,storeName,close,submit}:{orderId:string;storeId:string;storeName:string;close:()=>void;submit:(stars:number,comment:string)=>Promise<void>}){
  const [stars,setStars]=useState(5);const [comment,setComment]=useState("");const [submitting,setSubmitting]=useState(false);const [error,setError]=useState("");
  const save=async()=>{setSubmitting(true);setError("");try{await submit(stars,comment);}catch(reason){setError(reason instanceof Error?reason.message:"Your rating could not be saved.");setSubmitting(false);}};
  return <div className="px-modal-backdrop"><div className="px-modal px-rating-dialog" role="dialog" aria-modal="true" aria-label={`Rate ${storeName}`}><div className="px-modal-head"><div><span>DELIVERY COMPLETE</span><h2>How was {storeName}?</h2></div><button onClick={close} aria-label="Rate later"><Icon name="close"/></button></div><p>Your verified rating will appear when customers choose a seller.</p><div className="px-rating-stars" aria-label={`${stars} stars`}>{[1,2,3,4,5].map((value)=><button type="button" className={value<=stars?"active":""} onClick={()=>setStars(value)} key={value} aria-label={`${value} stars`}><Icon name="star"/></button>)}</div><label>Share an optional comment<textarea value={comment} maxLength={500} onChange={(event)=>setComment(event.target.value)} placeholder="Part quality, packing and delivery experience"/></label>{error?<p className="px-checkout-error" role="alert">{error}</p>:null}<div className="px-rating-actions"><button className="px-btn px-btn-outline" onClick={close} disabled={submitting}>Rate later</button><button className="px-btn px-btn-red" onClick={()=>void save()} disabled={submitting}>{submitting?"Saving…":"Submit verified rating"}</button></div><small>Order {orderId} · Store {storeId}</small></div></div>;
}
